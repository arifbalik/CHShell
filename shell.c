...
{
	int i;
	if(args->count > komutlar.count){
		CHTerminalPrint("Unknown Command\n");
		return 0;
	}
	if(args->count == 0){
		for(i = 0; i < komutlar.count; i++) {
			CHTerminalPrint("Command: ");
			CHTerminalPrint((char*)komutlar.cmds[i].cmd);
			CHTerminalPrint("\nDescpriction: ");
			CHTerminalPrint((char*)komutlar.cmds[i].desc);
			CHTerminalPrint("\n");
		}
		return 0;
	}
	for(i=0;i <= komutlar.count;i++){
		if(!strcmp(komutlar.cmds[i].cmd,args->args[0].val)){
			CHTerminalPrint("Command: ");
			CHTerminalPrint((char*)komutlar.cmds[i].cmd);
			CHTerminalPrint("\nDescpriction: ");
			CHTerminalPrint((char*)komutlar.cmds[i].desc);
